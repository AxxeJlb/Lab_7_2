...
struct ObjectTransform {
    ...
    struct {
        float min = -5.0f;
        float max = 5.0f;
    } zLimit; // Для объекта 1 (движение по Z)

    struct {
        float min = -3.0f;
        float max = 3.0f;
    } yLimit; // Для объекта 3 (движение по Y)

    // Ограничение вращения (в градусах)
    struct {
        float min = -90.0f;
        float max = 90.0f;
    } rotationLimit; // Для объекта 2 (вращение вокруг Z)
};
...
int main() {
    ...
    objectTransforms[1].zLimit = { -0.9f, 1.0f };
    objectTransforms[3].yLimit = { -0.5f, 0.03f };
    objectTransforms[2].rotationLimit = { 0.0f, 45.0f };
...
void processInput(GLFWwindow* window) {
    ...
    if (glfwGetKey(window, GLFW_KEY_I) == GLFW_PRESS) {
        ...
        if (objectTransforms[3].position.y > objectTransforms[3].yLimit.max)
            objectTransforms[3].position.y = objectTransforms[3].yLimit.max;
    }
    if (glfwGetKey(window, GLFW_KEY_K) == GLFW_PRESS) {
        objectTransforms[3].position.y -= moveSpeed;

        if (objectTransforms[3].position.y < objectTransforms[3].yLimit.min)
            objectTransforms[3].position.y = objectTransforms[3].yLimit.min;
    }
}
